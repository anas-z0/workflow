{
  ...
  outputs = { self, nixpkgs }:
    ...
    in rec {
      ...
      packages.x86_64-linux = builtins.mapAttrs (n: v: v.overrideAttrs (x: { src = lib.overrideDerivation packagesNoHash.x86_64-linux.${n}.src (x: { outputHash = (import ./hashes.nix).${n}; }); })) inputs.self.outputs.packagesNoHash;
      shell = let
